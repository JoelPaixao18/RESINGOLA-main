...
const baseImageUrl = "http://172.16.40.20/RESINGOLA-main/uploads/";
...
const Home = ({ route }) => {
  ...
  const fetchResidences = async () => {
    try {
      ...
      try {
        const response = await fetch('http://172.16.40.20/RESINGOLA-main/Backend/listar_residences.php');
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              const response = await fetch('http://172.16.40.20/RESINGOLA-main/Backend/deletar.php', {
                method: 'POST',
